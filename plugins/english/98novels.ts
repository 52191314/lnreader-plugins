...
class NinetyEightNovels implements Plugin.PluginBase {
  id = '98novels';
  name = '98Novels';
  site = 'https://98novels.com/';
  version = '1.0.0';
  icon = 'src/english/98novels/icon.png';
  ...
  async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
    ...
    loadedCheerio('p.wp-block-paragraph strong').each((_, element) => {
      ...
      const value = loadedCheerio(element).nextUntil('strong').text().trim();
      if (key === 'author:') novel.author = value;
      if (key === 'tl status:') {
        novel.status = STATUS_MAP[value.toLowerCase()] ?? NovelStatus.Unknown;
      }
    });

    const summaryHeading = loadedCheerio(
      'h4.wp-block-heading:contains("Summary:")',
    ).first();
...
export default new NinetyEightNovels();
